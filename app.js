...
document.write('<script src="https://cdn.jsdelivr.net/npm/artplayer/dist/artplayer.js"></script>');
...
if (dark) {
	document.write(
		'<style>* {box-sizing: border-box}body{color:rgba(255,255,255,.87);background-color:#333232}.mdui-theme-primary-' +
			main_color +
...
function title(path) {
	...
	if (model.is_search_page) $('title').html(`${document.siteName} - ${drive_name} - Search Result for ${model.q} `);
...
function list(path) {
	...
	requestListPath(path, { password: password }, successResultCallback, function (path) {
		$('#spinner').remove();
...
function onSearchResultItemClick(a_ele) {
	...
	$.post(`/${cur}:id2path`, { id: a_ele.id }, function (data) {
		if (data) {
			...
			dialog = mdui.dialog({
				title: '<i class="mdui-icon material-icons"></i>Open Folder',
		...
		dialog = mdui.dialog({
			...
			content:
				'o(╯□╰)o It may be because this item does not exist in the disk! It may also be because the file [Shared with me] has not been added to Personal Drive!',
			history: false,
			modal: true,
			closeOnEsc: true,
			buttons: [{ text: 'WTF ???' }],
...
<div class="mdui-textfield">
	<label class="mdui-textfield-label">Download Link 3</label>
...
function file_video(path) {
	...
	const urlSpecial = urlWithAuth + path;
	...
	const listPlayer = `
        <style>
            #playerList { margin: 20px 0; display:flex; justify-content: center; width: 100%; align-items: center; }
            #playerList a { display: block; margin: 0 10px; }
            #playerList .items { display: flex; justify-content: center; }
            #playerList img { width: 50px;}
            .text-preview, #buttonCopy { text-align: center; margin: 20px 0; }
            .openText { text-align: center; margin: 20px 0; display:inline-block; width: 100%; font-size: 18px; color: #131313; }
            .textNote { text-align: center; width: 100%; display: block; font-size: 14px; color: #747373; margin-top: 10px; font-style: italic; }
            .footer-text { margin-bottom: 50px; text-align: center; width: 100%; display: block; font-size: 14px; color: #747373; margin-top: 10px; font-style: italic; }
            #playerPreview {
                width: 100%;
                height: 450px;
            }
        ...
        <span class="textNote">Click mở trực tiếp trên IPhone, Android Phone ( thiết bị khác sử dụng link trực tiếp )</span>
        `;

	const content = `
    
<h3 class="text-preview">Preview  </h3>
...
<div class="mdui-container-fluid">
	...
	<!-- Fixed label -->
	<div class="mdui-textfield">
	  <label class="mdui-textfield-label">Download link 1</label>
	  <input class="mdui-textfield-input" type="text" value="${url}"/>
	</div>
	<div class="mdui-textfield">
...
<div class="mdui-container-fluid">
	...
	<br>
	<div class="mdui-textfield">
	  <label class="mdui-textfield-label">Download link 2</label>
	  <input class="mdui-textfield-input" type="text" value="${url}"/>
	</div>
	<div class="mdui-textfield">
	  <label class="mdui-textfield-label">HTML Reference address</label>
